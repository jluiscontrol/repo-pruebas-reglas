...
$consulta_fecha_ayer = "select sum(if(tt.afecta_cuenta='SUMA',if(ht.tipo_comision!='COMISION',(ht.valor+ht.comision_valor_porcentaje),ht.valor),if(tt.afecta_cuenta='RESTA',-(ht.valor),0))) as valor_en_cuenta,sum(ht.comision) as comision ,sum(ht.comision_valor_porcentaje) as comision_valor_porcentaje  from entidad en 
...
IF((select afecta_cuenta from tipo_transaccion where id=ht.tipo_transaccion)='SUMA',ht.valor,
   if((select afecta_cuenta from tipo_transaccion where id=ht.tipo_transaccion)='RESTA',-ht.valor,0)) as valor,ht.comision,ht.comision_valor_porcentaje,ht.comentario,ht.fecha_creada FROM historial_transacciones ht where tipo_transaccion in (SELECT id FROM tipo_transaccion where afecta_cuenta='SUMA' OR afecta_cuenta ='RESTA') and date(ht.fecha_creada)=date('{$fecha_actual}') and ht.id_entidad={$id} order by date(ht.fecha_creada),ht.id asc";

$resultado_ayer = mysqli_query($conn, $consulta_fecha_ayer) or die ("Algo ha ido mal en la consulta $resultado_ayer");
$resultado_actual = mysqli_query($conn, $consulta_fecha_actual) or die ("Algo ha ido mal en la consulta $consulta_fecha_actual");
